...
![Equity Curve vs Buy & Hold](wave_BTCUSDT_20260507T085158_equity.png)

## Returns & Growth
| Metric | Formula | Value |
|---|---|---|
| Total Return | $\frac{E_N - E_0}{E_0}$ | -183.56% |
| CAGR | $(E_N/E_0)^{P/N} - 1$ | 0.00% |
| Initial Capital | $E_0$ | $10,000.00 |
| Final Equity | $E_N$ | $-8,356.43 |

## Risk
| Metric | Formula | Value |
|---|---|---|
| Ann. Volatility | $\sigma(r) \times \sqrt{P}$ | 33.22% |
| Ann. Downside Vol | $\sigma(\min(r,0)) \times \sqrt{P}$ | 22.46% |
| Max Drawdown | $\min_t (E_t - \text{peak}_t)/\text{peak}_t$ | -177.13% |
| Max DD Duration (bars) | 156,930 |
| VaR 95% | $-Q_{0.05}(r)$ | 0.00% |
| CVaR 95% (ES) | $-\mathbb{E}[r \mid r < \text{VaR}_{95}]$ | 0.01% |
| VaR 99% | $-Q_{0.01}(r)$ | 0.18% |
| CVaR 99% (ES) | $-\mathbb{E}[r \mid r < \text{VaR}_{99}]$ | 0.52% |
| Ulcer Index | $\sqrt{N^{-1}\sum D_t^2}$ | 0.7854 |
| Skewness | $\frac{1}{N}\sum((r_t-\bar r)/\sigma)^3$ | 4.4011 |
| Excess Kurtosis | $\frac{1}{N}\sum((r_t-\bar r)/\sigma)^4 - 3$ | 956.3651 |

## Risk-Adjusted
| Metric | Formula | Value |
|---|---|---|
| Sharpe Ratio | $(\bar{r} \cdot P - R_f)\,/\,(\sigma\sqrt{P})$ | -3.4091 |
| Sortino Ratio | $(\text{CAGR} - \text{MAR})\,/\,(\sigma_d\sqrt{P})$ | -5.0427 |
...
| Exposure % | 3.3% |
| Win Rate | 25.72% |
| Profit Factor | 0.7732 |
| Avg Win | $98.96 |
| Avg Loss | $-44.33 |
| Expectancy / Bar | -0.0011% |

## Per-Regime Breakdown
| Regime | Time % | PnL Contribution | Sharpe | N Bars |
|---|---|---|---|---|
| BREAKOUT | 2.3% | $-6,215.53 | -8.58 | 3,899 |
| MEAN_REVERSION | 81.7% | $-1,000.94 | -1.45 | 139,164 |
| BREAKDOWN | 3.5% | $-480.80 | -33.92 | 5,940 |
| NEUTRAL | 12.6% | $-10,659.16 | -12.85 | 21,398 |

## Monthly Returns
Best month: 13717.15%  |  Worst month: -601.01%  |  Positive months: 30.0%
...
![Wave Regime Timeline](wave_BTCUSDT_20260507T085158_wave_timeline.png)
...
![Regime Hit Rate vs Horizon](wave_BTCUSDT_20260507T085158_wave_hit_rate.png)
...
![Per-Regime Mean Forward Return vs Horizon](wave_BTCUSDT_20260507T085158_wave_fwd_return.png)
...
![Regime Transition Matrix](wave_BTCUSDT_20260507T085158_wave_transition.png)
...
![Regime × Tide Bias Confusion](wave_BTCUSDT_20260507T085158_wave_confusion.png)
...
|---|---|---|---|
| BREAKDOWN | -0.107% | 0.001% | 0.010% |
| BREAKOUT | 0.001% | 0.007% | 0.000% |
| MEAN_REVERSION | 0.019% | 0.000% | 0.007% |
| NEUTRAL | 0.004% | 0.002% | -0.002% |
